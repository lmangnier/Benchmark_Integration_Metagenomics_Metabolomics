#!/usr/bin/env Rscript
#This function returns the redundancy for the PLS regression 
#The function takes as argument the data as produced by code_simulation*.R and a microbiome normalization
...
pls.results = lapply(1:1000, function(rep){
	...
	pls = mixOmics::pls(normalized.species, metabolites, ncomp=min.col, mode="regression") #species and metabolites can be switched if needed
	compute.redundancy.pls(normalized.species, metabolites, pls)
})

saveRDS(pls.results, paste0("results_PLS_regression_",microbiome.norm,".RDS"))
